...
class Homepage extends Component {
    ...
    async fetchData() {
        try {
            const response = await getHomeData(this.props.token, 0, 1, this.state.exploreAll)
    ...
    loadMoreData = async () => {
        ...
        try {
            const response = await getHomeData(this.props.token, this.state.dataPage, 1, this.state.exploreAll)
            let data = shuffle(response.data)
